...
public class CitaService {
    ...
    private void validarDisponibilidadYHorarios(Long veterinarioId, Servicio servicio, Instant fechaHora, Long citaAExcluirId) {
        ...
        for (DisponibilidadVeterinario disp : disponibilidades) {
            if (disp.getActivo()) {
                LocalTime dispInicio = disp.getHoraInicio().atZone(zone).toLocalTime();
                LocalTime dispFin = disp.getHoraFin().atZone(zone).toLocalTime();
                if (!horaInicioCita.isBefore(dispInicio) && !horaFinCita.isAfter(dispFin)) {
        ...
        List<BloqueoVeterinario> bloqueos = bloqueoRepositoryPort.findByVeterinarioIdAndFecha(veterinarioId, inicioDia);
        for (BloqueoVeterinario bloq : bloqueos) {
            if (!horaInicioCita.isBefore(bloq.getHoraFin().atZone(zone).toLocalTime()) &&
                    !bloq.getHoraInicio().atZone(zone).toLocalTime().isBefore(horaFinCita)) {
                continue;
            }
            if (horaInicioCita.isBefore(bloq.getHoraFin().atZone(zone).toLocalTime()) &&
                    bloq.getHoraInicio().atZone(zone).toLocalTime().isBefore(horaFinCita)) {
